...
pub use roots::{set_manifest_roots, ManifestRoots};
pub use status::{AgentRemoteStatus, ManifestUpdateStatus};
pub use version::{ManifestVersion, MANIFEST_ENGINE_VERSION};
...
/// Process identity for a recognized agent.
...
pub fn identify_agent_in_job(job: &crate::ForegroundJob) -> Option<(Agent, String)> {
...
pub fn identify_agent_process(process_name: &str) -> Option<Agent> {
    identify_agent(process_name)
}

pub fn detect_agent_optional(
    agent: Option<Agent>,
    input: manifest::DetectionInput<'_>,
) -> AgentDetection {
    detect_agent_with_osc(agent, input.screen, input.osc_title, input.osc_progress)
}
...
fn normalized_process_name(process: &crate::ForegroundProcess) -> String {
...
fn process_priority(process: &crate::ForegroundProcess, normalized_name: &str) -> u8 {
...
mod tests {
    ...
    fn foreground_process(pid: u32, name: &str, argv: &[&str]) -> crate::ForegroundProcess {
        crate::ForegroundProcess {
            pid,
            name: name.to_string(),
            argv0: None,
    ...
    fn identify_agent_in_job_ignores_invalid_windows_cursor_install_paths() {
        ...
        let lookalike = crate::ForegroundJob {
            process_group_id: 123,
            processes: vec![foreground_process(
                123,
                "node.exe",
        ...
        let job = crate::ForegroundJob {
            ...
            processes: vec![foreground_process(
                ...
                &[
                    ...
                    "-NoExit",
                    "-Command",
                    "if ($null -eq $function:prompt) { function global:prompt { 'PS> ' } }",
                ],
            )],
        };
